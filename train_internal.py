...
from scene.exist_prob_pruner import ExistProbPruner

from utils.loss_utils import ssim
from lpipsPyTorch import lpips as lpips_fn

def ensure_nchw01(x: torch.Tensor) -> torch.Tensor:
    """Return tensor as NCHW in [0,1]. Accepts HWC, CHW, or NCHW."""
    if x.dim() == 3:
        if x.shape[-1] in (1, 3):  # HWC -> NCHW
            x = x.permute(2, 0, 1).unsqueeze(0)
        else:                      # CHW -> NCHW (add batch)
            x = x.unsqueeze(0)
    elif x.dim() == 4:
        if x.shape[1] not in (1, 3) and x.shape[-1] in (1, 3):  # NHWC -> NCHW
            x = x.permute(0, 3, 1, 2)
        ...
        with torch.no_grad():
            ...
            if iteration < opt_args.iterations:
                ...
                if not args.disable_auto_densification and iteration <= args.densify_until_iter:
                   pruner.step(gaussians, batched_screenspace_pkg, batched_strategies, iteration)
        ...
        validation_configs = (
            {
                ...
                "num_cameras": len(test_cameras) if test_cameras is not None else 0,
            },
            {
                "name": "train",
                "cameras": train_cameras,
                "num_cameras": max(len(train_cameras) // args.llffhold if train_cameras is not None else 0, args.bsz),
            },
        )

        diff1 = abs(iteration - 7000)
        diff2 = abs(30000 - iteration)
        if diff1 <= 7 or diff2 <= 7:
            ssim_flag = True
        else:
            ssim_flag = False
        ...
        for config in validation_configs:
            if config["cameras"] and len(config["cameras"]) > 0:
                ...
                for idx in range(1, num_cameras + 1, args.bsz):
                    ...
                    for camera_id, (image, gt_camera) in enumerate(zip(batched_image, batched_cameras)):
                        ...
                        if idx + camera_id < num_cameras + 1:
                            ...
                            if ssim_flag == True:
                                _img = ensure_nchw01(image)
                                _gt = ensure_nchw01(gt_image)
                                ssim_test += ssim(_img, _gt).mean().double()
                                lpips_test += lpips_fn(_img * 2.0 - 1.0, _gt * 2.0 - 1.0).mean().double()
                ...
                if ssim_flag == False:
                    ...
                    utils.print_rank_0(
                        "\n[ITER {}] Evaluating {}: PSNR {} SSIM {} LPIPS {}".format(
