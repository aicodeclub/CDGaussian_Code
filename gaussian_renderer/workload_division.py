...
class DivisionStrategyFinal:
    ...
    def get_local2j_ids_ext(self, means2D, radii, raster_settings, cuda_args):
        dist_global_strategy_tensor = (
                torch.tensor(self.division_pos, dtype=torch.int, device=means2D.device)
                * utils.TILE_X
        )
    ...
    def get_topk_idx(self, means2D, radii, raster_settings, cuda_args, max_k):
        dist_global_strategy_tensor = (
            torch.tensor(self.division_pos, dtype=torch.int, device=means2D.device)
            * utils.TILE_X
        )
        ...
        topk_idx = diff_gaussian_rasterization._C.get_topk_idx(*args)
...
class DivisionStrategyHistoryFinal:
    def __init__(self, dataset, world_size, rank):
        ...
        if dataset.cameras is not None:
