...
class GaussianModel:
    ...
    def add_points(self,
                   ...
                   new_send_to_gpui_cnt: torch.Tensor):

        C_dc, S = self._features_dc.shape[1], self._features_dc.shape[2]
        C_rest = self._features_rest.shape[1]
        n_new = new_sh.shape[0]
        device = new_sh.device

        new_features_dc = torch.zeros((n_new, C_dc, S), device=device)
        new_features_rest = torch.zeros((n_new, C_rest, S), device=device)

        new_features_dc[:, :, 0] = new_sh[:, :C_dc]
        new_features_rest[:, :, 0] = new_sh[:, C_dc:]

        d = {
            'xyz': new_xyz,  # [n_new,3]
            'f_dc': new_features_dc,  # [n_new,C_dc,S]
            'f_rest': new_features_rest,  # [n_new,C_rest,S]
            'opacity': new_probs.unsqueeze(-1),  # [n_new,1]
            'scaling': new_scaling,  # [n_new,scale_dim]
            'rotation': new_rotation  # [n_new,rot_dim]
        }

        optim_tensors = self.cat_tensors_to_optimizer(d)
        ...
        self.sum_visible_count_in_one_batch = torch.zeros((N,), device=device)


        orig = self.send_to_gpui_cnt
        new_cnt = new_send_to_gpui_cnt

        if orig.dim() == 2:
            D = orig.size(1)
            if new_cnt.dim() == 1:
                new_cnt = new_cnt.unsqueeze(-1)  # -> [N_new,1]
            if new_cnt.dim() == 2 and new_cnt.size(1) == 1:
                new_cnt = new_cnt.expand(-1, D)  # -> [N_new, D]
        elif orig.dim() == 1:
            if new_cnt.dim() == 2 and new_cnt.size(1) == 1:
                new_cnt = new_cnt.squeeze(-1)  # -> [N_new]
            assert new_cnt.dim() == 1, \
                f"send_to_gpui_cnt is 1D，but new_send_to_gpui_cnt is {new_cnt.dim()}D"
        else:
            raise RuntimeError(f"not support send_to_gpui_cnt.dim()={orig.dim()}")

        self.send_to_gpui_cnt = torch.cat((orig, new_cnt), dim=0)
