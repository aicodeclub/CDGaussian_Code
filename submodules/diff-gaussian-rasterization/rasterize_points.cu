...
__global__ void get_topk_idx_kernel(
	int P,
	int height,
	int width,
	int world_size,
	...
	int max_k)
{
    ...
    auto opts = torch::TensorOptions().dtype(torch::kInt32).device(means2D.device());
    // 用-1初始化
    torch::Tensor topk_idx = torch::full({P, max_k}, -1, opts);

	get_topk_idx_kernel << <(P + ONE_DIM_BLOCK_SIZE - 1) / ONE_DIM_BLOCK_SIZE, ONE_DIM_BLOCK_SIZE >> >(
		P,
		H,
		W,
		mp_world_size,
